    ...
    <>
      ...
      >
        <div className='main-container py-6 flex justify-between items-center'>
          ...
          {/* Logo */}
          <Link to='/'>
            <img src={Logo} alt="Digvijay Wadekar — Home" className='h-10 w-auto' />
          </Link>

          {/* Right side: social icons + resume + hamburger */}
          <div className='flex items-center gap-2'>

            {/* GitHub — p-2 enlarges tap target */}
            <a href="https://github.com/Digvijaywadekar2004" target="_blank" rel="noopener noreferrer"
            ...
            {/* LinkedIn */}
            <a href="https://linkedin.com/in/digvijaywadekar" target="_blank" rel="noopener noreferrer"
            ...
            <a
              ...
              className="uppercase font-heading text-sm font-semibold px-5 py-1.5 rounded-full border border-white text-white hover:bg-white hover:text-black transition-all duration-300 ml-1"
            >
              Resume
            </a>
            ...
            <button
              ...
              className="menubar flex flex-col gap-1.5 cursor-pointer ml-2 bg-transparent border-0 p-2"
            >
      ...
      {/* Fullscreen Menu */}
      <div
        className={`fixed z-20 inset-0 bg-black text-white flex flex-col items-center justify-center text-3xl gap-8 transition-transform duration-500 ${menuOpen ? "translate-y-0 pointer-events-auto" : "-translate-y-full pointer-events-none"}`}
        onClick={() => setMenuOpen(false)}
      >
        {/* Menu links */}
        <Link to="/" className='menu-link'>Home</Link>
        <Link to="/projects" className='menu-link'>Projects</Link>
        <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" className='menu-link'>Resume</a>
        <button
          onClick={() => {
            setMenuOpen(false);
            openContact();
          }}
          className='menu-link bg-transparent border-0 cursor-pointer text-white'
        >
          Contact
        </button>
      </div>
